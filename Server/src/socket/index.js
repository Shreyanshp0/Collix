...
export function initializeSocketServer(httpServer, {
	config = createSocketConfig(), groupService: groups = groupService, messageService: messages = messageService,
	presenceService: presence = presenceService, rateLimitGuard, logger = defaultLogger,
} = {}) {
	assertLogger(logger);
	...
	io.on('connection', (socket) => {
		...
		registerChatSocketHandlers({ io, socket, groupService: groups, messageService: messages, presenceService: presence, rateLimitGuard, logger });
